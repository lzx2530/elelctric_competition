...
3. 平衡外环与 K230 新视觉帧同步（设计值 30 Hz），内部的步进位置环仍以 1 kHz 运行。首次先用 `+50 mm -> -50 mm` 验证方向，之后再依次实测调整反馈、IMU 前馈与摩擦破除参数。
4. 该版外环使用仿真参数 `Kp=24.15`、`Kd=7.95`、`Ki=24.50`、倾角限制 `+/-6 deg`，并假定视觉链路延迟为 2 帧。如果实测延迟超过 3 帧，先降低外环带宽再测试。
5. 空车完成一圈、A 点启停线识别和编码器里程；再以 `0.25 m/s` 带球测试。
6. 最后启用 IMU 加速度前馈，按实车方向校验纵向加速度符号。
