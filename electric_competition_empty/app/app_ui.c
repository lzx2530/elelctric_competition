    ...
    const imu_snapshot_t *imu)
{
    ...
    oled_printf(&g_oled, 0U, 8U, "E%.2f B%02X S%u", chassis->line_error, chassis->line_bits, chassis->line_state);
    oled_printf(&g_oled, 0U, 16U, "T%u X%d Y%d", turret->target_valid ? 1U : 0U, turret->x_error, turret->y_error);
